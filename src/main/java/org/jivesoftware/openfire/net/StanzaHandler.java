...
public abstract class StanzaHandler {
    ...
    private String getStreamHeader() {
        ...
        sb.append("\" from=\"");
        sb.append(connection.getConnectionDomain());
        sb.append("\" id=\"");
    ...
    protected void createSession(XmlPullParser xpp) throws XmlPullParserException, IOException {
        ...
        if (validateHost() && isHostUnknown(domain)) {
            ...
            Log.warn("Closing session due to incorrect hostname in stream header. Domain: " + domain +
                    ". Connection: " + connection);
        }
    ...
    private boolean isHostUnknown(String host) {
        ...
        if (DomainManager.getInstance().isRegisteredDomain(host)) 
        {
     ...
     *
     */
    abstract boolean createSession(String namespace, String domain, XmlPullParser xpp, Connection connection)
