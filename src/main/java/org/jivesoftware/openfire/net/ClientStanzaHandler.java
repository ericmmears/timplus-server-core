...
public class ClientStanzaHandler extends StanzaHandler {
    ...
    boolean validateHost() {
        return JiveGlobals.getBooleanProperty("xmpp.client.validate.host",true);
    }
    ...
    @Override
    boolean createSession(String namespace, String domain, XmlPullParser xpp, Connection connection)
            throws XmlPullParserException {
        if ("jabber:client".equals(namespace)) {
            // The connected client is a regular client so create a ClientSession
        	connection.setConnectionDomain(domain);
            session = LocalClientSession.createSession(domain, xpp, connection);
            return true;
        }
        return false;
    }
