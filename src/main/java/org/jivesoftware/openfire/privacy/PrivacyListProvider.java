...
public class PrivacyListProvider {
     ...
     *         with the specified name does not exist.
     */
    public PrivacyList loadPrivacyList(String username, String domain, String listName) {
        // If there are no privacy lists stored, this method is a no-op.
        if (!databaseContainsPrivacyLists.get()) {
            return null;
        }
        ...
        try {
            ...
            privacyList = new PrivacyList(username, domain, listName, isDefault, listElement);
        }
        catch (Exception e) {
     ...
     */
    public PrivacyList loadDefaultPrivacyList(String username, String domain) {
        // If there are no privacy lists stored, this method is a no-op.
        if (!databaseContainsPrivacyLists.get()) {
            return null;
        }
        ...
        try {
            ...
            xmlReader = xmlReaders.take();
            Element listElement = xmlReader.read(new StringReader(listValue)).getRootElement();
            privacyList = new PrivacyList(username, domain, listName, true, listElement);
        }
        catch (Exception e) {
            Log.error(e.getMessage(), e);
        }
