            ...
            if (ap.authorize(username, principal)) {
                ...
                try {
                    ...
                    if(JiveGlobals.getBooleanProperty("xmpp.auth.autoadd",false)) {
                        ...
                        try {
                            UserManager.getInstance().createUser(username, StringUtils.randomString(8), null, null, DomainResolver.resolveUserDomain(username));
                            if (Log.isDebugEnabled()) {
                        ...
                        {                            
                        	if (Log.isDebugEnabled())
                        		Log.error("Domain " + DomainResolver.resolveUserDomain(username) + " does not exist in the system.");
                            return false;     	
                        }
                    }
                    return false;
