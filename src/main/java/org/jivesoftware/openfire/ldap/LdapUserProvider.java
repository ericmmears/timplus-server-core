...
public class LdapUserProvider implements UserProvider {
    ...
    @Override
    public User createUser(String username, String password, String doamin, String name, String email)
    ...
    @Override
    public synchronized Collection<User> getUsersByDomain(String domainName) {
        ...
        return filterAllUsersByDomain(domainName);
    }    
    
    private Collection<User> filterAllUsersByDomain(String domainName)
    {
    	final Collection<User> filteredUsers = new ArrayList<>();
    	for (User user : this.allUsers)
    		if (user.getDomain().equals(domainName))
    			filteredUsers.add(user);
    	
    	return Collections.unmodifiableCollection(filteredUsers);
    }
            ...
            userlist = userlist.stream()
                .filter(user ->
                    !GroupManager.getInstance().getGroups(
                        XMPPServer.getInstance().createJID(user,  DomainResolver.resolveUserDomain(user) , null))
                    .isEmpty())
