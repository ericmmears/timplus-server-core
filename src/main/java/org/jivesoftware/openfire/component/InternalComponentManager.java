...
public class InternalComponentManager extends BasicModule implements ClusterEventListener, ComponentManager, RoutableChannelHandler {
    ...
    public void addComponent(String subdomain, Component component) throws ComponentException {
        synchronized (routables) {
        	final String serviceDomainName = buildServiceDomainName(subdomain, component);
            
            RoutableComponents routable = routables.get(serviceDomainName);
            ...
            Log.debug("InternalComponentManager: Registering component for domain: " + serviceDomainName);
            
            JID componentJID = new JID(serviceDomainName);
            ...
            if (routable != null) {
                ...
                routables.put(serviceDomainName, routable);
            ...
            try {
                ...
                Log.debug("InternalComponentManager: Component registered for domain: " + serviceDomainName);
            }
            catch (Exception e) {
                ...
                if (routable.numberOfComponents() == 0) {
                    ...
                    routables.remove(serviceDomainName);
                    // Remove the route
    ...
    public void removeComponent(String subdomain, Component component) {
        ...
        synchronized (routables) {
            Log.debug("InternalComponentManager: Unregistering component for domain: " + serviceDomainName);
            RoutableComponents routable = routables.get(serviceDomainName);
            routable.removeComponent(component);
            if (routable.numberOfComponents() == 0) {
                routables.remove(serviceDomainName);
        ...
        synchronized (routables) {
            ...
            if (routable != null) {
                ...
                String serverName = componentJID.getDomain();
                    ...
                    routable = routables.get(serverName);
                    if (routable != null) {
                        return routable.getComponents();
                }
            }
...
//        sendPacket(component, iq);
    ...
    protected String buildServiceDomainName(String subdomain, Component component)
    {
    	final String serviceDomainName;
        if (component instanceof MultiUserChatService)
        	serviceDomainName = subdomain + "." + MultiUserChatService.class.cast(component).getDomain();
        else
        	serviceDomainName = subdomain + "." + serverDomain;
