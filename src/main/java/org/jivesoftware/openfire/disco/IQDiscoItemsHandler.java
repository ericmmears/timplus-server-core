...
public class IQDiscoItemsHandler extends IQHandler implements ServerFeaturesProvider, ClusterEventListener,
    ...
    public IQ handleIQ(IQ packet) {
        ...
        if (itemsProvider != null) {
            ...
            Iterator<DiscoItem> itemsItr = itemsProvider.getItems(name, node, packet.getFrom(), domain);
            if (itemsItr != null) {
    ...
    public void initialize(XMPPServer server) {
        ...
        for (String domainName : DomainManager.getInstance().getDomainNames(true))
        	setProvider(domainName, getServerItemsProvider());
            ...
            public Iterator<DiscoItem> getItems(String name, String node, JID senderJID, String domain) {
                if (node != null) {
                    ...
                    if (serverNodeProviders.get(node) != null) {
                        return serverNodeProviders.get(node).getItems(name, node, senderJID, domain);
                    }
                    return null;
                }
                if (name == null) {
                    return getServerItems(domain).iterator();
                }
                else {
     ...
     */
    public List<DiscoItem> getServerItems(String domain) {
        List<DiscoItem> answer = new ArrayList<>();
        for (ClusteredServerItem item : serverItems.values()) 
        {
        	final DiscoItem discoItem = new DiscoItem(item.element);
        	if (StringUtils.isEmpty(domain) || discoItem.getJID().toString().toLowerCase().contains(domain.toLowerCase()))
        		answer.add(discoItem);
        }
        return answer;
    }
}
