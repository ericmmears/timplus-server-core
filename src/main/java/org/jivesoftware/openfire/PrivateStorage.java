...
public class PrivateStorage extends BasicModule {
    ...
    public void add(String username, Element data) {
        ...
        final JID owner = XMPPServer.getInstance().createJID( username, DomainResolver.resolveUserDomain(username), null );
    ...
    public Element get(String username, Element data)
    {
        ...
        {
            final PEPServiceManager serviceMgr = XMPPServer.getInstance().getIQPEPHandler().getServiceManager();
            final PEPService pepService = serviceMgr.getPEPService( XMPPServer.getInstance().createJID( username, DomainResolver.resolveUserDomain(username), null ) );
            if ( pepService != null )
            {
