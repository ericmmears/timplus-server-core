...
public class MUCServicePropertyEventDispatcher {
     ...
     */
    public static void dispatchEvent(String service, String domain, String property, EventType eventType, Map<String, Object> params) {
        for (MUCServicePropertyEventListener listener : listeners) {
            try {
                switch (eventType) {
                    case property_set: {
                        listener.propertySet(service, domain, property, params);
                        break;
                    }
                    case property_deleted: {
                        listener.propertyDeleted(service, domain, property, params);
                        break;
                    }
                    default:
                        break;
