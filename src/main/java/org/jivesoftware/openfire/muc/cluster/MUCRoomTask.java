...
public abstract class MUCRoomTask<V> implements ClusterTask<V> {
    ...
    protected MUCRoomTask(LocalMUCRoom room) {
        ...
        domain = room.getMUCService().getDomain();
    }

    public LocalMUCRoom getRoom() {
        MultiUserChatService mucService = XMPPServer.getInstance().getMultiUserChatManager().getMultiUserChatService(subdomain, domain);
        if (mucService == null) {
