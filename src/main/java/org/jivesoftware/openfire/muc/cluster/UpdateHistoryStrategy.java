...
public class UpdateHistoryStrategy implements ClusterTask<Void> {
    ...
    public UpdateHistoryStrategy(String serviceName, String domainName, HistoryStrategy historyStrategy) {
    ...
    @Override
    public void run() {
        MultiUserChatServiceImpl mucServer = (MultiUserChatServiceImpl) XMPPServer.getInstance().getMultiUserChatManager().getMultiUserChatService(serviceName, domainName);
