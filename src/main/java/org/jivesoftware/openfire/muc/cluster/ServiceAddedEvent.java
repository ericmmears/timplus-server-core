...
public class ServiceAddedEvent implements ClusterTask<Void> {
    ...
    public ServiceAddedEvent(String subdomain, String domain, String description, Boolean isHidden) {
    ...
    public void run() {
        ...
        if (!XMPPServer.getInstance().getMultiUserChatManager().isServiceRegistered(subdomain, domain)) {
            MultiUserChatService service = new MultiUserChatServiceImpl(subdomain, domain, description, isHidden);
