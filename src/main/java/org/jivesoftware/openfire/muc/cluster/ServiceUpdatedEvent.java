...
public class ServiceUpdatedEvent implements ClusterTask<Void> {
    ...
    public ServiceUpdatedEvent(String subdomain, String domain) {
    ...
    @Override
    public void run() {
        MultiUserChatService service = XMPPServer.getInstance().getMultiUserChatManager().getMultiUserChatService(subdomain, domain);
        if (service != null) {
            if (service instanceof MultiUserChatServiceImpl) {
                MUCPersistenceManager.refreshProperties(subdomain, domain);
