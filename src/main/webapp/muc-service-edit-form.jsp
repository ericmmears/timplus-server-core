...
<%  // Get parameters
    ...
    if (!create && !webManager.getMultiUserChatManager().isServiceRegistered(mucname, mucdomain)) {
    ...
    if (!create && mucdesc == null) {
        mucdesc = webManager.getMultiUserChatManager().getMultiUserChatService(mucname, mucdomain).getDescription();
    }
    ...
    if (save) {
        ...
        if (errors.size() == 0) {
            ...
            if (!create) {
                webManager.getMultiUserChatManager().updateMultiUserChatService(mucname, mucdomain, mucname, mucdesc);
            }
            else {
                try {
                    webManager.getMultiUserChatManager().createMultiUserChatService(mucname, mucdomain, mucdesc, false);
                }
            ...
            if (muccleanupdays != null) {
                MUCPersistenceManager.setProperty(mucname, mucdomain, "unload.empty_days", muccleanupdays);
            }

            if (ParamUtils.getParameter(request, "muckeep") != null) {
                if (ParamUtils.getParameter(request, "muckeep").equalsIgnoreCase("on")) {
                    MUCPersistenceManager.setProperty(mucname, mucdomain, "unload.empty_days", "0");
                }
            }
