    ...
    if (save) {
        ...
        if (errors.size() == 0) {
            ...
            if (!AdminManager.getAdminProvider().isReadOnly()) {
                boolean isCurrentAdmin = AdminManager.getInstance().isUserAdmin(user.getUsername(), user.getDomain(), false);
                if (isCurrentAdmin && !isAdmin) {
                    AdminManager.getInstance().removeAdminAccount(user.getUsername(), user.getDomain());
                }
                else if (!isCurrentAdmin && isAdmin) {
                    AdminManager.getInstance().addAdminAccount(user.getUsername(), user.getDomain());
                }
            }
...
<fieldset>
    ...
    <tbody>
        ...
        <tr>
            ...
            <td>
                <input type="checkbox" name="isadmin"<%= AdminManager.getInstance().isUserAdmin(user.getUsername(), user.getDomain(), false) ? " checked='checked'" : "" %>>
