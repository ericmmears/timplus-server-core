...
<%  // Get parameters //
    ...
    if (create) {
        ...
        if (errors.size() == 0) {
            try {
                User newUser = webManager.getUserManager().createUser(username, password, name, email, domain);

                if (!AdminManager.getAdminProvider().isReadOnly()) {
                    boolean isCurrentAdmin = AdminManager.getInstance().isUserAdmin(newUser.getUsername(), newUser.getDomain(), false);
                    if (isCurrentAdmin && !isAdmin) {
                        AdminManager.getInstance().removeAdminAccount(newUser.getUsername(), newUser.getDomain());
                    }
                    else if (!isCurrentAdmin && isAdmin) {
                        AdminManager.getInstance().addAdminAccount(newUser.getUsername(), newUser.getDomain());
                    }
                }
    ...
    <tbody>
        <tr>
            ...
            <% } else if (errors.get("domain") != null) { %>
                <fmt:message key="user.create.invalid_domain" />   
            <% } else if (errors.get("unmanagedDomain") != null) { %>
                <fmt:message key="user.create.unmanaged_domain" />                               
...
<form name="f" action="user-create.jsp" method="get">
    ...
    <div class="jive-contentBox">
        ...
        <tr>
            <td width="1%" nowrap><label for="domaintf"><fmt:message key="user.create.domain" />:</label> *</td>
            <td width="99%">
